class Scatterplot {
    ...
    initVis() {
        ...
        // Reset selection on brush start
        function resetBrush() {
            filteredRegionIds = [];
            updateSelection(0);
            updateRegionData();
        ...
        function isBrushed(brush_coords, cx, cy) {
            let x0 = brush_coords[0][0], x1 = brush_coords[1][0], y0 = brush_coords[0][1], y1 = brush_coords[1][1];
            return x0 <= cx && cx <= x1 && y0 <= cy && cy <= y1;
        }

        // Update brushed points
    ...
    updateVis() {
        ...
        vis.yScale.domain([d3.min(vis.yearFilteredData, vis.yValue) - Math.abs(d3.max(vis.yearFilteredData, vis.yValue) * 0.1), d3.max(vis.yearFilteredData, vis.yValue) + d3.max(vis.yearFilteredData, vis.yValue) * 0.1]);
        ...
        vis.regressionPoints = vis.xScale.domain().map(d => {
            return {
                x: d, y: vis.linearRegressionLine(d)
            };
        });
        vis.line = d3.line()
            .x(d => vis.xScale(d.x))
    ...
    renderVis() {
        ...
        const normalR = 6, myR = 8, normalRHover = 8, myRHover = 10;
        ...
        vis.regression
            .datum(vis.regressionPoints)
            .attr('d', vis.line);

        // Update confidence interval
        vis.ci
            .datum(vis.intervalData())
            .attr("d", d3.area()
                .x(function (d) {
                    return vis.xScale(d.x)
                })
                .y0(function (d) {
                    return vis.yScale(d.left)
                })
                .y1(function (d) {
                    return vis.yScale(d.right)
                }));
    ...
    normDev(p) {
        ...
        let a0 = 3.3871328727963666080e+0, a1 = 1.3314166789178437745e+2, a2 = 1.9715909503065514427e+3,
            a3 = 1.3731693765509461125e+4, a4 = 4.5921953931549871457e+4, a5 = 6.7265770927008700853e+4,
            a6 = 3.3430575583588128105e+4, a7 = 2.5090809287301226727e+3,

            b1 = 4.2313330701600911252e+1, b2 = 6.8718700749205790830e+2, b3 = 5.3941960214247511077e+3,
            b4 = 2.1213794301586595867e+4, b5 = 3.9307895800092710610e+4, b6 = 2.8729085735721942674e+4,
            b7 = 5.2264952788528545610e+3,

            c0 = 1.42343711074968357734e+0, c1 = 4.63033784615654529590e+0, c2 = 5.76949722146069140550e+0,
            c3 = 3.64784832476320460504e+0, c4 = 1.27045825245236838258e+0, c5 = 2.41780725177450611770e-1,
            c6 = 2.27238449892691845833e-2, c7 = 7.74545014278341407640e-4,

            d1 = 2.05319162663775882187e+0, d2 = 1.67638483018380384940e+0, d3 = 6.89767334985100004550e-1,
            d4 = 1.48103976427480074590e-1, d5 = 1.51986665636164571966e-2, d6 = 5.47593808499534494600e-4,
            d7 = 1.05075007164441684324e-9,

            e0 = 6.65790464350110377720e+0, e1 = 5.46378491116411436990e+0, e2 = 1.78482653991729133580e+0,
    ...
    inverseT(p, df) {
        ...
        if (y > 0.05 + a) {
            ...
            y = a * y * y;
            if (y > 0.002) y = exp(y) - 1; else y = 0.5 * y * y + y;
        } else {
    ...
    // Calculate the confidence bands of the data
